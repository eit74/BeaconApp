...
import de.rvwbk.eit74.beaconapp.restconnection.object.BeaconObject;
...
public class RadarActivity extends AppCompatActivity {
    ...
    private Triple<String,Integer,TaskObject> tri;
    ...
    public RadarActivity(){
        ...
        this.broadcastReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if(beaconID.equals(UserData.getInstance().getNextBeaconMM())) {
                    unregisterReceiver();
                    Intent i = new Intent(meself, DefaultQuestActivity.class);
                    i.putExtra("TaskID", UserData.getInstance().getNextTask());
                    startActivity(i);
                } else {
                    Toast.makeText(getApplicationContext(),"Dieser Beacon ist noch nicht dran!",Toast.LENGTH_LONG).show();
                }
            }
        };
    }
     ...
     * @return next task id
     */
    private Triple<String, Integer, TaskObject> getNextTask () {
        ...
        // parse all Tasks
        List<Triple<String, Integer, TaskObject>> taskList = new ArrayList<>();
        for (ObjectInterface oi : taskObjectList) {
            if (oi instanceof TaskObject) {
                ...
                //                                                       // ID   // POS // task
                Triple<String, Integer, TaskObject> triple = new Triple(task.get("id"), thing, task);
                taskList.add(triple);
            } else {
        ...
        List<Triple<String, Integer, TaskObject>> completedTaskObjects = new LinkedList<>();
        for (String completedTask : completedTasks) {
            for (Triple<String, Integer, TaskObject> triple : taskList) {
        ...
        for (Triple<String, Integer, TaskObject> completed : completedTaskObjects) {
            taskList.remove(completed);
        }

        // select lowest
        Triple<String, Integer, TaskObject> next = taskList.get(0);
        for (Triple<String, Integer, TaskObject> task : taskList) {
            if (next.y > task.y) {
                next = task;
            }
        }
        ...
        AsyncConnection ac3 = new AsyncConnection();
        BeaconObject bo  = (BeaconObject)ac3.get(new BeaconObject(),next.z.get("beacon_id"));
        UserData.getInstance().setNextBeaconMM(bo.get("major-minor"));
        return next;
    }
